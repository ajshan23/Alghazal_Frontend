...
export const fetchShops = async (search?: string, page: number = 1, limit: number = 30) => {
    try {
...
export const fetchCategories = async (search?: string, page: number = 1, limit: number = 30) => {
    try {
...
export const fetchVehicles = async (search?: string, page: number = 1, limit: number = 30) => {
    try {
    ...
    try {
        const response = await BaseService.get(`/bills`, {
            params: {
                ...
                category,
                shop,
                vehicle, // This should match backend expectation
                paymentMethod,
                employee, // Add employee filter
            },
        })
        return response.data
    } catch (error) {
...
export const exportBillToExcel = async ({
    ...
    try {
        ...
        // Check if response is actually an error (sometimes errors come as JSON in blob)
        ...
        // Return the blob for external handling (saveAs)
        return response.data
