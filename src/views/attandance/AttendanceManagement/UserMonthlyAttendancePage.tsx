...
const UserMonthlyAttendancePage = () => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        if (responseData?.attendance && Array.isArray(responseData.attendance)) {
          ...
          // Handle separated attendance data
      ...
      // Sort attendance by date and type for consistent display
  ...
  const renderCalendar = () => {
    ...
    // Create a map for quick lookup - handle multiple records per day
  ...
  const renderTotals = () => {
    ...
    if (totals.overall) {
      // New API response format with separated totals
      return (
      ...
      // Legacy format or simple totals
      return (
        <div className="mt-6">
