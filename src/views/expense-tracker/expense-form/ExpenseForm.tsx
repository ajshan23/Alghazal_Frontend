...
const ExpenseForm = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (expenseId) {
          ...
          console.log("API Response:", JSON.stringify(expenseResponse.data, null, 2));

          // Extract the actual materials data from the complex structure
          const materials = expenseResponse.data.materials.map((m: any) => {
            // Check different possible structures
            const materialData = m._doc || m.__parentArray || m;
            return {
              description: materialData.description,
              date: new Date(materialData.date),
              invoiceNo: materialData.invoiceNo,
              amount: materialData.amount,
              supplierName: materialData.supplierName || '',
              supplierMobile: materialData.supplierMobile || '',
              supplierEmail: materialData.supplierEmail || '',
              documentUrl: materialData.documentUrl,
              documentKey: materialData.documentKey,
              file: null // Initialize file as null for existing documents
            };
          });
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    ...
    try {
      ...
      const materialsData = values.materials.map((m, index) => ({
        description: m.description,
      ...
      values.materials.forEach((material, index) => {
        if (material.file) {
          // Add index to filename to help backend identify which material the file belongs to
          const fileName = `material-${index}-${material.file.name}`;
          formData.append('files', material.file, fileName);
        }
      });
      ...
      if (isEditMode && expenseId) {
        ...
        // Check if projectId is available before creating expense
        if (!projectId) {
          toast.push(
            <Notification title="Error" type="danger">
              Project ID is missing
      ...
      navigate(`/app/expense-view/${expenseId}`);
    } catch (error: any) {
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg">
          ...
          <p className="text-red-700 dark:text-red-300">
            Unable to load the expense form. Please ensure you're accessing this page from a valid project.
          </p>
          <Button
            className="mt-4"
        ...
        {({ values, setFieldValue, errors, touched }) => (
          <Form className="space-y-6">
                            ...
                            {material.documentUrl && !material.file && (
                              <div className="mt-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    // Clear the document URL when user wants to remove the existing file
                                    setFieldValue(`materials[${index}].documentUrl`, undefined);
                                    setFieldValue(`materials[${index}].documentKey`, undefined);
                                  }}
                            ...
                            {material.file && (
                              <div className="mt-2 text-sm text-green-600 dark:text-green-400">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    setFieldValue(`materials[${index}].file`, null);
                                  }}
                                  className="ml-4 text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300"
                                >
                                  Remove
            ...
            <div className="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-4 py-3 flex justify-between shadow-lg">
              <Button
                type="button"
                onClick={() => navigate(`/app/project-view/${projectId}`)}
                variant="plain"
              >
                Cancel
              </Button>
